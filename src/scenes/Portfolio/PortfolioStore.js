...
class PortfolioStore {
  @observable balances = observable.map({});
  @observable changes = observable.map({});
  @observable editMode = 'crypto';
  @observable rawEditedBalances = observable.map({}); // Temporary when user enters edit mode
  ...
  @computed get editedBalances() {
    ...
    return observable.map(balances);
  }
  ...
  @action fromJSON = (jsonData) => {
    ...
    if (parsed.balances) {
      ...
      const balances = {};
      observable.map(parsed.balances).forEach((amount, currency) => {
